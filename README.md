...
假设模型不需要变换直接在世界坐标系下渲染（注意tinyrender里的变换并不是标准的变换流程，代码实现上会有所区别)

**视图变换**

首先相机有一个向量up，一般朝上，先确定z方向，z=eye(相机位置)-center(视点)，单位化，用z和up叉积求x，单位化，z和x叉积求y，单位化。现在可以构造正交矩阵[x,y,z]，世界坐标系变换到相机坐标系，S=invert(C)*B，C构成相机坐标系的正交标准基即[x,y,z]，B为世界坐标系的标准基为单位阵，正交矩阵的逆为其转置，S=transpose(C)。等等，别忘了相机并不是在原点，坐标变换过去后要减去相机中心eye，用齐次坐标描述这个变换，4*4矩阵View，左上3*3块为S，最后一列为[center ,1]

![1747539140151](image/readme/1747539140151.png)

**投影变换**

这里不做详细推导，简略说明投影矩阵是怎么来的（基于opengl摄像机形式）

如果近平面中心在摄像机坐标系下xy不为0，通过齐次矩阵M0平移使x,y=0

假设相机坐标系下，znear是n,zfar是f，第一次变换，坐标[x,y,z]->[x * n/z, y * n/z, n/z]，[f, z]->[n/f, 1]，通过齐次矩阵M1实现：[x,y,z,1]->[x,y,1,z/n]，视锥体变换成裁剪立方体

然后通过齐次矩阵M2往z移动-(n/f+1)/2，这样z向[n/f,1]->[n/2f-1/2,1/2-n/2f]

再通过齐次矩阵M3缩放裁剪立方体变换成[-1,1]正方体
