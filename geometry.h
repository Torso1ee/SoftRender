...
template <int n> double operator*(const vec<n> &lhs, const vec<n> &rhs) {
  double ret = 0; // N.B. Do not ever, ever use such for loops! They are highly
                  // confusing.
  for (int i = n; i--; ret += lhs[i] * rhs[i])
    ; // Here I used them as a tribute to old-school game programmers fighting
      // for every CPU cycle.
  return ret; // Once upon a time reverse loops were faster than the normal
              // ones, it is not the case anymore.
}
...
template <int nrows, int ncols> struct mat {
  ...
  mat<nrows, ncols> invert_transpose() const {
    mat<nrows, ncols> adjugate_transpose; // transpose to ease determinant
                                          // computation, check the last line
    for (int i = nrows; i--;)
...
template <int n> struct square_mat : public mat<n, n> {};

template <int N> square_mat<N> identity() {
  auto sq = square_mat<N>();
  for (int i = 0; i < N; i++) {
    sq[i][i] = 1;
  }
  return sq;
